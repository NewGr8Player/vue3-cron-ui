<template>
  <a-radio-group class="radio-group" v-model:value="type">
    ...
    <a-radio :value="TYPE.STEP" v-if="isStep">
      {{ label.step[0] }}
      <input-number v-model="step[0]" :range="[...stepLeft]" />
      {{ label.step[1] }}
      <input-number v-model="step[1]" :range="[...stepRight]" />
      {{ label.step[2] }}
    </a-radio>
    <a-radio :value="TYPE.WELL" v-if="isWell">
      {{ label.well[0] }}
      <input-number v-model="well[1]" :range="[...wellLeft]" />
      {{ label.well[1] }}
      <input-number
        v-model="well[0]"
        :range="[...wellRight]"
    ...
    <a-radio :value="TYPE.WEEKDAY" v-if="isWeekday">
      {{ label.weekday[0] }}
      <input-number v-model="weekday" :range="[...rangeStart]" />
      {{ label.weekday[1] }}
    </a-radio>
  ...
  computed: {
    ...
    value() {
      switch (this.type) {
        ...
        case TYPE.SPECIFY:
          return this.specify.length
            ? // eslint-disable-next-line vue/no-side-effects-in-computed-properties
              this.specify.sort((a, b) => a - b).join(this.type)
            : `${this.specifies[0].value}`
        default:
          return ''
      }
    },
  ...
  watch: {
    modelValue: {
      handler(modelValue) {
        ...
        if (
          [TYPE.EMPTY, TYPE.UNSPECIFIC, TYPE.LAST_DAY, TYPE.LAST_WEEKDAY, TYPE.EVERY].includes(
            value,
          )
        ) {
          this.type = value
